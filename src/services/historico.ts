...
export type AcaoHistorico = "unidade" | "marcacoes" | "criar" | "renomear" | "excluir" | "contexto" | "equipe";
export type ComandoMapa = { mapa: MapaServico } & (
  ...
  | { acao: "equipe"; equipeId: string }
  | { acao: "criar" | "excluir" }
);
export interface EventoHistorico {
  id: string; plantaId?: string; acao: AcaoHistorico; userId: string; mapaId: string; mapaNome: string;
...
export function validarMarcacoes(marcacoes: Marcacoes, unidades: readonly string[] = Object.keys(UNIDADE_BY_ID)) {
  if (Object.entries(marcacoes).some(([id, status]) => !unidades.includes(id)
...
export function gravarComHistorico(db: Firestore, uid: string, obraId: string, comando: ComandoMapa, unidades: readonly string[] = Object.keys(UNIDADE_BY_ID)): Promise<void> {
  ...
  const metadados = { schemaVersion: CURRENT_SCHEMA_VERSION, obraId, ...escopoPlanta, atualizadoEm: serverTimestamp(), atualizadoPor: uid, ultimoEventoId: evento.id };
  if (acao === "unidade") {
    antes = { [comando.unidadeId]: mapa.marcacoes[comando.unidadeId] ?? null };
    depois = { [comando.unidadeId]: comando.status };
    validarMarcacoes(depois, unidades);
    ...
    validarMarcacoes(antes, unidades); validarMarcacoes(depois, unidades);
    ...
    if (comando.equipeId && !/^[a-zA-Z0-9_-]{1,128}$/.test(comando.equipeId)) throw new ErroOperacional("validacao", "Equipe inválida.");
    antes = { equipeId: mapa.equipeId ?? "" }; depois = { equipeId: comando.equipeId };
    if (antes.equipeId === depois.equipeId) return Promise.resolve();
    lote.update(referencia, { ...metadados, equipeId: comando.equipeId });
  } else if (acao === "criar") {
